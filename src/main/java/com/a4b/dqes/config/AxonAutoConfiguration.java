...
import com.a4b.core.server.hibernate_validator.CommandValidationInterceptor;
import com.a4b.dqes.aop.event.EventPublishingAspect;
 ...
 * @author Josh Long
 */
@org.springframework.context.annotation.Configuration(enforceUniqueMethods = false)
@AutoConfigureAfter(value = {DatabaseConfiguration.class, AxonJpaAutoConfiguration.class})
...
public class AxonAutoConfiguration implements BeanClassLoaderAware {
	...
	public AxonAutoConfiguration(EventProcessorProperties eventProcessorProperties,
			SerializerProperties serializerProperties, TagsConfigurationProperties tagsConfigurationProperties,
	...
	private Serializer buildSerializer(RevisionResolver revisionResolver,
		...
		case JAVA:
			return JacksonSerializer.builder().revisionResolver(revisionResolver).build();
		case XSTREAM:
		case DEFAULT:
		default:
			XStreamSerializer xStreamSerializer = XStreamSerializer.builder().revisionResolver(revisionResolver)
    ...
    /**
     * Fix the error: org.axonframework.common.AxonConfigurationException: An
     * unsecured XStream instance allowing all types is used. It is strongly
     * recommended to set the security context yourself instead!
     */
    @Bean
    @Qualifier("defaultAxonXStream")
    public XStream defaultAxonXStream() {
    ...
    @Bean
    public CommandValidationInterceptor commandValidationInterceptor() {
    	return new CommandValidationInterceptor();
    }
    ...
    public SimpleCommandBus commandBus(TransactionManager txManager, Configuration axonConfiguration,
            DuplicateCommandHandlerResolver duplicateCommandHandlerResolver, CommandValidationInterceptor commandValidationInterceptor) {
        ...
        // Dang ky custom interceptors
        commandBus.registerHandlerInterceptor(commandValidationInterceptor);
        return commandBus;
    }
    ...
    public SimpleQueryBus queryBus(Configuration axonConfiguration, TransactionManager transactionManager) {
    	return SimpleQueryBus.builder().transactionManager(transactionManager)
								...
								.spanFactory(axonConfiguration.spanFactory())
    							.messageMonitor(axonConfiguration.messageMonitor(QueryBus.class, "queryBus")).build();
    }
    ...
    @Bean
    public EventPublishingAspect eventPublishingAspect() {
    	return new EventPublishingAspect();
    }
}
